	...
	for JOB_TYPE in "${ARR_JOB[@]}"; do
		for BS in "${ARR_BS[@]}";do
			for QD in "${ARR_QD[@]}"; do
				...
				LOG_PATH=/mnt/data/${SRC_VOL}/swap-2G/${JOB_TYPE}-QD${QD}-BS${BS}/timelog
                ...
                mkdir -p $SUMMARY_PATH $LOG_PATH $STAT_PATH $SWAP_PATH
				...
				for numcont in $(seq $MIN_NUM_CONT $MAX_NUM_CONT); do
					...
					for NS in $(seq 0 3); do
						...
						for CONT_ID in $(seq $start $end); do
							...
							stat-per-process $PID 10 $RUN_TIME $SWAP_FILE
					
							echo "fio --name ${DEV}${DEV_ID}-cont${NUM_CONT}ID${INTERNAL_ID} --thread=1 --ioengine=libaio --iodepth=$QD --rw=$JOB_TYPE --bs=$BS --time_based --runtime=$RUN_TIME --filename=/dev/${DEV}${DEV_ID} --output=$SUMMARY_FILE --log_avg_msec=$LOG_MSEC --write_lat_log=$LOG_FILE --write_bw_log=$LOG_FILE --write_iops_log=$LOG_FILE"	
							docker exec -d ID${CONT_ID} bash -c "fio --name ${DEV}${DEV_ID}-cont${NUM_CONT}ID${INTERNAL_ID} --thread=1 --ioengine=libaio --iodepth=$QD --rw=$JOB_TYPE --bs=$BS --time_based --runtime=$RUN_TIME --filename=/dev/${DEV}${DEV_ID} --output=$SUMMARY_FILE --log_avg_msec=$LOG_MSEC --write_lat_log=$LOG_FILE --write_bw_log=$LOG_FILE --write_iops_log=$LOG_FILE"
							docker exec -d ID${CONT_ID} bash -c "dockerstat 10 15000 $STAT_FILE"
						done
					done

#					sleep 240
#					func_check_done $TOTAL_NUM_CONT
