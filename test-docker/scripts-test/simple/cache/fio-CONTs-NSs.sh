...
func_simple()
{
	for JOB_TYPE in "${ARR_JOB[@]}"; do
		for BS in "${ARR_BS[@]}";do
			for QD in "${ARR_QD[@]}"; do
				mkdir -p /mnt/data/${SRC_VOL}/NS4/${JOB_TYPE}-QD${QD}-BS${BS}
                mkdir -p /mnt/data/${SRC_VOL}/NS4/${JOB_TYPE}-QD${QD}-BS${BS}/summary
				mkdir -p /mnt/data/${SRC_VOL}/NS4/${JOB_TYPE}-QD${QD}-BS${BS}/timelog
				mkdir -p /mnt/data/${SRC_VOL}/NS4/${JOB_TYPE}-QD${QD}-BS${BS}/dockerstat
				mkdir -p /mnt/data/${SRC_VOL}/NS4/${JOB_TYPE}-QD${QD}-BS${BS}/blktrace
				...
				for numcont in $(seq $MIN_NUM_CONT $MAX_NUM_CONT); do
					...
					for NS in $(seq 0 3); do
						...
						for CONT_ID in $(seq $start $end); do
							...
							LOG_FILE=/mnt/data/${SRC_VOL}/NS4/${JOB_TYPE}-QD${QD}-BS${BS}/timelog/${DEV}${DEV_ID}-cont${NUM_CONT}ID${INTERNAL_ID}
							STAT_FILE=/mnt/data/${SRC_VOL}/NS4/${JOB_TYPE}-QD${QD}-BS${BS}/dockerstat/${DEV}${DEV_ID}-cont${NUM_CONT}ID${INTERNAL_ID}.stat
							...
							docker exec -d ID${CONT_ID} bash -c "fio --name ${DEV}${DEV_ID}-cont${NUM_CONT}ID${INTERNAL_ID} \
								...
								--output=/mnt/data/${SRC_VOL}/NS4/${JOB_TYPE}-QD${QD}-BS${BS}/summary/${DEV}${DEV_ID}-cont${NUM_CONT}ID${INTERNAL_ID}.summary \
						...
						BLKTRACE_PATH=/mnt/data/${SRC_VOL}/NS4/${JOB_TYPE}-QD${QD}-BS${BS}/blktrace/
						...
						for CONT_ID in $(seq $start $end); do
							INTERNAL_ID=$(($CONT_ID-$NS*$NUM_CONT))
							BLKTRACE_FILE=/mnt/data/${SRC_VOL}/NS4/${JOB_TYPE}-QD${QD}-BS${BS}/blktrace/${DEV}${DEV_ID}-cont${NUM_CONT}ID${INTERNAL_ID}.blktrace
