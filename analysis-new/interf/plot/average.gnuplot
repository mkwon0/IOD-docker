...
do for [job_id=1:words(ARR_JOB_TYPE)] {
	do for [target_id=1:words(ARR_TARGET_TYPE)]{
		FILE_PNG=sprintf("%s/figs/average-%s-QD1-BS4k-%s.png",OUTPUT_PATH,word(ARR_JOB_TYPE,job_id),word(ARR_TARGET_TYPE,target_id))
		TITLE=sprintf("%s-fio-%s-QD1-BS4k-Interf-noCache",word(ARR_TARGET_TYPE,target_id),word(ARR_JOB_TYPE,job_id))
		INPUT=sprintf("%s/%s-QD1-BS4k/summary-parse/cont1ID1-summary-%s.dat",INPUT_PATH,word(ARR_JOB_TYPE,job_id),word(ARR_TARGET_TYPE,target_id))
		if (target_id == 1) {
			YLABEL="Bandwidth (MB/s)"
		} else {
			YLABEL="Latency (us)"
		...
		set title TITLE
		set ylabel YLABEL textcolor rgb Y1COLOR
		plot for [col=2:5] INPUT u 0:col w lp ls (col-1) t "NS".(col-1)
		print FILE_PNG
	}
}
