...
NUM_NS="4-noCache"
#ARR_JOB=["read","randread", "randwrite"]
ARR_JOB=["write"]
		...
		for QD in ARR_QD:
			for NUM_CONT in ARR_NUM_CONT:
				for NS in ARR_NS:
					...
					bw_file_name = "/mnt/data/resource/NS"+str(NUM_NS)+"/"+str(REQ_TYPE)+"-QD"+str(QD)+"-BS4k/summary-parse/nvme3n"+str(NS)+"-cont"+str(NUM_CONT)+"-bw.dat"
					lat_file_name = "/mnt/data/resource/NS"+str(NUM_NS)+"/"+str(REQ_TYPE)+"-QD"+str(QD)+"-BS4k/summary-parse/nvme3n"+str(NS)+"-cont"+str(NUM_CONT)+"-lat.dat"
					bw_file = open(bw_file_name,'w')
					lat_file = open(lat_file_name,'w')
					bw_cont_id, lat_cont_id = [],[]
					for CONT_ID in xrange(1,NUM_CONT+1):
						LOG_PATH= "/mnt/data/resource/NS"+str(NUM_NS)+"/"+str(REQ_TYPE)+"-QD"+str(QD)+"-BS4k/summary/nvme3n"+str(NS)+"-cont"+str(NUM_CONT)+"ID"+str(CONT_ID)+".summary"
